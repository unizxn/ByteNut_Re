...
class BytenutRenewal:
    ...
    def detect_connection_error(self, sb):
        """检测 Chrome 是否显示连接错误页面（SOCKS5→Xray 转发失败等）"""
        try:
    ...
    def navigate_and_wait(self, sb, url, reconnect_time=5):
        """导航到 URL 并检测是否为 Chrome 错误页（非正常页面）"""
        try:
    ...
    def navigate_to_panel(self, sb, server_id):
        ...
        time.sleep(5)
        # 检测连接错误（SOCKS5→Xray 转发可能失败）
        is_error, error_kw = self.detect_connection_error(sb)
        if is_error:
            raise Exception(f"无法访问面板页面（{error_kw}），请检查代理配置或网络连接")
        self.remove_overlay_ads(sb)
    ...
    def run(self):
        ...
        for idx, (user, pwd) in enumerate(accounts, 1):
            ...
            account_done = False
            for browser_attempt in range(1, 4):  # 浏览器重启重试（连接错误时）
                connection_error = False
                try:
                    proxy_display = PROXY_DISPLAY if PROXY else "直连"
                    self.log(f"🌐 代理: {proxy_display}"
                             + (f" (第{browser_attempt}/3次)" if browser_attempt > 1 else ""))
                    ...
                    with SB(
                        ...
                        try:
                            ...
                            self.log(f"⚠️ 登录页导航失败: {nav_err}")
                            if any(kw in str(nav_err) for kw in [
                                "ERR_CONNECTION", "无法访问", "页面未加载"
                            ]):
                        ...
                        sb.uc_open_with_reconnect(URL_HOMEPAGE,
                                                  reconnect_time=6)
                        time.sleep(8)
                        ...
                        if not ready:
                            self.send_tg("❌", "面板加载失败", user,
                                         server_id, state, expiry_str,
                                         screenshot=self.shot(
                                             sb,
                                             f"panel_fail_{idx}.png"))
                            account_done = True
                            break
            ...
            if not account_done and connection_error:
                self.log(f"❌ 账号 {masked_user} "
                         f"经过 3 次浏览器重启仍无法连接")
                self.send_tg("❌", "连接失败", user, "未知", "未知",
                             "3次重试后仍无法访问网站")
